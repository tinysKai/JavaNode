...
对应于本篇文章就是`postProcessAfterInitialization`方法会接收到此容器内所有bean实例化,初始化之后的回调(每个bean调用一次),然后spring aop模块使用自己新生成的代理返回给spring从而覆盖了原来的bean对象.但此时这个对象的beanName以及beanId都没变化,只是实例变化了.下次调用时获取到的bean变成了代理后的bean.
