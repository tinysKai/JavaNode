...
- **漏桶算法对突发流量支持较小(因流出速率是一定的),而令牌桶支持突发流量**
...
Guava是基于令牌算法的,而且支持预消费

```java
//一个阻塞式的限流,acquire
public class GuavaLimit {
    public static void main(String[] args) {
        //create方法的参数代表每秒生成几个令牌
        RateLimiter limit = RateLimiter.create(1);
        for (int i = 1; i < 4; i++) {
            double time = limit.acquire(i);
...
//执行结果
//wait for 0.0 , 预消费,无需等待,消耗一个令牌,此时令牌数为-1
//wait for 0.998592 先填补了上一次的预消费,在令牌数为0时执行预消费2个令牌,此时令牌数为-2
//wait for 1.94812  等到2s来填补上一次的预消费
```

```java
//一个获取失败就放弃的限流,tryAcquire
public class GuavaLimitWithTry {
    public static void main(String[] args) throws InterruptedException {
        RateLimiter limit = RateLimiter.create(1);
        for (int i = 1; i < 9; i++) {
            boolean allowed = limit.tryAcquire(1);
            if (allowed){
                System.out.println(i + " get the permission");
            }
            Thread.sleep(500L);
        }
    }
...
//5 get the permission
//7 get the permission
```
...
static RateLimiter create(SleepingStopwatch stopwatch, double permitsPerSecond) {
    //注意这里的maxBurstSeconds写死为1,意味着最多保留1s的令牌数,比如你每秒生成10个令牌,就是最多保存10个令牌
    RateLimiter rateLimiter = new SmoothBursty(stopwatch, 1.0 /* maxBurstSeconds */);
    //设置速度方法会最终调到下面的doSetRate方法
